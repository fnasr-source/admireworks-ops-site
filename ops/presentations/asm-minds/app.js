/**
 * ASM Minds x Admireworks — Strategy Presentation Engine
...
(function () {
  ...
  var isSyncing = false; /* prevents broadcast feedback loops */
  ...
  function renderScreenshot(c) {
    ...
    return '<div class="slide-screenshot">' +
      '<img src="' + esc(c.screenshot) + '" alt="' + esc(c.screenshot_caption || 'Screenshot') + '" class="screenshot-img">' +
  ...
  function renderCover(c) {
    return '<div class="cover-logos">' +
      ...
      '<span style="font-family:var(--font-headline);font-size:clamp(20px,3vw,40px);font-weight:700;color:white;">ASM Minds</span>' +
      '</div>' +
    ...
    rows.forEach(function (row) {
      ...
      cols.forEach(function (col) {
        ...
        if (val === undefined) {
          ...
          else if (cl.indexOf('product') >= 0) val = row.product_type || row.product;
          else if (cl.indexOf('asm') >= 0) val = row.asm_minds || row.asm;
          else if (cl.indexOf('competitor') >= 0) val = row.competitors;
          else if (cl.indexOf('note') >= 0) val = row.notes;
        }
        html += '<td>' + esc(val || '\u2014') + '</td>';
      });
      html += '</tr>';
  ...
  function renderPersona(c) {
    ...
    if (c.objection_ar) html += '<div class="persona-quote" style="background:var(--asm-blue);margin-top:12px;">' + esc(c.objection_ar) + '</div>';
    html += '</div>';
    return html;
  }
  ...
  function renderFunnel(c) {
    var html = makeHeader(c);
    if (c.funnel_image) html += '<div class="funnel-image-wrap"><img src="' + esc(c.funnel_image) + '" alt="Funnel"></div>';
    if (Array.isArray(c.stages_summary)) {
      html += '<div class="funnel-stages">';
      c.stages_summary.forEach(function (s) {
        html += '<div class="funnel-stage"><div class="funnel-stage-number">' + esc(s.number) + '</div>' +
          '<div class="funnel-stage-name">' + esc(s.name) + '</div>' +
          (s.name_ar ? '<div class="funnel-stage-name-ar">' + esc(s.name_ar) + '</div>' : '') +
          '<div class="funnel-stage-channels">' + esc(s.channels) + '</div>' +
          '<div class="funnel-stage-objective">' + esc(s.objective) + '</div></div>';
      });
      html += '</div>';
    }
    return html;
  ...
  function goToSlide(index) {
    ...
    if (channel && !isSyncing) channel.postMessage({ type: 'goto', index: index });

    // Scroll viewport to top on mobile
    var viewport = $('#viewport');
    if (viewport && window.innerWidth <= 768) viewport.scrollTop = 0;
  }
  ...
  function setupNavigation() {
    ...
    window.addEventListener('hashchange', function () {
      ...
      if (!isNaN(idx) && idx >= 0 && idx < slides.length && idx !== currentIndex) goToSlide(idx);
    });
  }
  ...
  function setupBroadcastChannel() {
    try {
      channel = new BroadcastChannel('asm-minds-strategy');
      channel.onmessage = function (e) {
        if (e.data.type === 'goto') {
          /* Any tab can receive — just prevent re-broadcasting */
          isSyncing = true;
  ...
  function setupPresenterMode() {
    ...
    if (btnPresenter) {
      btnPresenter.addEventListener('click', function () {
        ...
        btnPresenter.classList.toggle('active', isPresenter);
        if (notesPanel) notesPanel.classList.toggle('visible', isPresenter);
        /* Re-update notes content when toggling on */
        if (isPresenter) {
          var notesEl = $('#notesContent');
