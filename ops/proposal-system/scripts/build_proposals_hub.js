...
function mapRepoPath(relPath) {
  const normalized = asPosix(String(relPath || '').trim()).replace(/^\/+/, '');
  if (!normalized) return '';
  if (normalized.startsWith('Proposals/_Outgoing/')) return normalized.replace('Proposals/_Outgoing/', 'ops/proposal-system/_Outgoing/');
  if (normalized.startsWith('Proposals/_Proposal-System/')) return normalized.replace('Proposals/_Proposal-System/', 'ops/proposal-system/');
  if (normalized.startsWith('Proposals/')) return normalized.replace('Proposals/', 'clients/');
  return normalized;
}

function mapPublicSitePath(relPath) {
  const normalized = asPosix(String(relPath || '').trim()).replace(/^\/+/, '');
  if (!normalized) return '';
  if (normalized.startsWith('Proposals/_Outgoing/')) return normalized.replace('Proposals/_Outgoing/', 'ops/proposal-system/_Outgoing/');
  if (normalized.startsWith('Proposals/_Proposal-System/')) return normalized.replace('Proposals/_Proposal-System/', 'ops/proposal-system/');
...
function fileExists(root, relPath) {
  ...
  return fs.existsSync(path.join(root, mapRepoPath(relPath)));
}
...
function buildUrls(repoSlug, siteBaseInput) {
  ...
  return {
    blob: (relPath) => `${baseBlob}${encodeURI(mapPublicSitePath(relPath))}`,
    raw: (relPath) => `${baseRaw}${encodeURI(mapPublicSitePath(relPath))}`,
    preview: (relPath) => `${basePreview}${encodeURI(mapPublicSitePath(relPath))}`
  };
}
...
function normalizeLink(relPath, urls) {
  ...
  if (/^https?:\/\//i.test(relPath)) {
    try {
      ...
      if (url.hostname === 'ops.admireworks.com') {
        url.pathname = `/${mapPublicSitePath(url.pathname)}`;
        return url.toString();
      }
      return relPath;
    } catch {
...
function main() {
  ...
  const registryPath = path.join(root, 'ops', 'proposal-system', 'proposal-registry.csv');
  const crmPath = path.join(root, 'ops', 'proposal-system', 'proposal-crm.csv');
  ...
  const outputs = [
    path.join(root, 'ops', 'proposal-system', '_Outgoing', '_internal-crm', 'index.html'),
    path.join(root, 'ops', 'dashboards', 'proposals', 'index.html')
  ];
