> # RETIRED
>
> This system is no longer used and building from it produces a document that
> does not match anything we ship. The canon is
> **`docs/PROPOSAL-DESIGN-SYSTEM.md`**; use the **`aw-proposal-build`** skill.
>
> Written for Antigravity, and its `@font-face` rules point at a
> `_Proposal-System/` path that does not resolve, so Jaymont and Akkurat Pro
> silently fell back to Georgia and Helvetica in every proposal built from it.
> Its `clients/Meguiars-Egypt/proposal-v2/` reference implementation does not
> exist either.
>
> Kept for reference and for the reasoning in it that is still good. Do not
> follow its build steps.
...
>
> **Reference Standard:** The [Ein Abayaa Growth Partnership Proposal](file:///Users/user/Documents/IDE%20Projects/Internal%20AW%20SOP/drafts/Ein%20Abaya%20—%20Growth%20Partnership%20Proposal.pdf) — an 11-page, bilingual, premium proposal with mockups, comparison tables, and consultative pricing.
...
2. Check for existing research in `clients/{Client}/`
3. Check Knowledge Items for relevant industry or pattern data
4. Output: structured `00-Client-Brief.md` saved to `clients/{Client}/proposal/`
...
### Output:
- Save all research as `01-Research-Raw.md` in `clients/{Client}/proposal/`
- Save screenshots to `clients/{Client}/proposal/assets/research/`
...
Save all assets to: `clients/{Client}/proposal/assets/`
```
assets/
...
<head>
  ...
  <link rel="stylesheet" href="../../ops/proposal-playbook/templates/proposal-base.css">
...
:root {
  ...
  --prop-bg-dark: #1a2e1a;     /* Deep forest green (Ein Abayaa style) */
...
node "ops/proposal-playbook/scripts/generate-proposal-pdf.js" \
  "clients/{Client}/proposal/proposal.html" \
  "clients/{Client}/proposal/{Client} — Growth Partnership Proposal.pdf"
```
...
Compare against the Ein Abayaa reference on:
...
- **Meguiar's Egypt V2**: `clients/Meguiars-Egypt/proposal-v2/`
- 16 slides, 6.82 MB PDF, 454K EGP value stack at 32K/mo price point

---

## Reusable build learnings

See [references/founder-imagery-and-sharing.md](references/founder-imagery-and-sharing.md) for founder-as-face imagery (two-stage enhance-then-render from soft photos), WhatsApp/social link previews, brand architecture (house + collection), Egyptian luxury Arabic, no internal tool names in client decks, and offer mechanics. Captured from the Maison Ruby / Aurea build, June 2026.
